 ...
 */
    ...
    class CAdminLogin extends AdminBaseModel
    {
        ...
        function doModel()
        {
            switch( $this->action ) {
                case('login_post'):     //post execution for the login
                                        ...
                                        if( !$admin ) {
                                            osc_add_flash_error_message( sprintf(_m('Sorry, incorrect username or password. <a href="%s">Have you lost your password?</a>'), osc_admin_base_url(true) . '?page=login&amp;action=recover' ), 'admin');
                                            $this->redirectTo( osc_admin_base_url(true)."?page=login" );
                                        }
                ...
                case('recover_post'):   if( defined('DEMO') ) {
                                        ...
                                        if( (osc_recaptcha_private_key() != '') ) {
                                            if( !osc_check_recaptcha() ) {
                                                osc_add_flash_error_message( _m('The reCAPTCHA is invalid'), 'admin');
        ...
        {
            $login_admin_title = osc_apply_filter('login_admin_title', osc_page_title());
            // $login_admin_url   = osc_apply_filter('login_admin_url', osc_get_http_referer());
