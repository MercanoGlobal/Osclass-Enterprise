        ...
        function doModel()
        {
            ...
            switch ($this->action) {
                ...
                case('edit'):           // calling the edit view
                                        ...
                                        $actions[] = '<a class="btn btn-green float-left" href="'.osc_admin_base_url(true).'?page=users&action=login_user&amp;id='.$aUser['pk_i_id'] . '&amp;' . $csrf_token . '" target="_blank">'. __('Log in as user') .'</a>';
                                        if( $aUser['i_items'] > 0) {
                                            $actions[] = '<a class="btn float-left" target="_blank" href="' . osc_admin_base_url(true) . '?page=items&userId='. $aUser['pk_i_id'] .'">' . __('View user items') . '</a>';
                                        }
                ...
                case ('login_user'):    //login as user
                                        ...
                                        osc_add_flash_ok_message(sprintf(_m('Successfully logged in as %s'), $user['s_name']));
