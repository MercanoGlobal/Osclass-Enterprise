 ...
 */
    ...
    class CWebItem extends BaseModel
    {
        ...
        function doModel()
        {
            ...
            switch( $this->action ) {
                case 'item_add': // post
				    if( !osc_user_publishing_enabled() && !osc_is_admin_user_logged_in() ) {  // checks if Users are allowed to publish listings
                        osc_add_flash_error_message( _m('Only authorized users are allowed to publish listings.') );
                        $this->redirectTo( osc_base_url() );
                    } elseif( osc_reg_user_post() && $this->user == null ) {
                ...
                case 'item_add_post':
                    ...
                    if($success!=1 && $success!=2) {
                        ...
                        if($success==1) {
                            osc_add_flash_ok_message( _m('Check your e-mail inbox to validate your listing.') );
                        } else if(osc_moderate_admin_post()) {
                            osc_add_flash_ok_message(_m('Your listing will be published after a moderator approves it.'));
                        } else {
                ...
                case 'item_edit':   // edit item
                    // if( !osc_user_publishing_enabled() && !osc_is_admin_user_logged_in() ) {  // checks if Users are allowed to publish/edit listings
                        // osc_add_flash_error_message( _m('Only authorized users are allowed to edit listings.') );
                    ...
                    if (count($item) == 1) {
                        ...
                        if($success==1) {
                            ...
                            if(osc_moderate_admin_edit()) {
                                osc_add_flash_ok_message(_m('Your listing will be published after a moderator approves the changes.'));
                            } else {
                ...
                break;
                case 'activate':
                    if( !osc_user_publishing_enabled() && !osc_is_admin_user_logged_in() ) {  // checks if Users are allowed to publish/activate listings
                        osc_add_flash_error_message( _m('Only authorized users are allowed to activate listings.') );
                        $this->redirectTo( osc_user_list_items_url() );
                    }
                    $secret = Params::getParam('secret');
                    $id     = Params::getParam('id');
                ...
                break;
                case 'renew':
                    if( !osc_user_publishing_enabled() && !osc_is_admin_user_logged_in() ) {  // checks if Users are allowed to publish/renew listings
                        osc_add_flash_error_message( _m('Only authorized users are allowed to renew listings.') );
                        $this->redirectTo( osc_user_list_items_url() );
                    }
                    $secret = Params::getParam('secret');
                    $id     = Params::getParam('id');
                ...
                break;
                case 'item_delete':
                    if( !osc_user_publishing_enabled() && !osc_is_admin_user_logged_in() ) {  // checks if Users are allowed to publish/delete listings
                        osc_add_flash_error_message( _m('Only authorized users are allowed to delete listings.') );
                        $this->redirectTo( osc_user_list_items_url() );
                    }
                    $secret = Params::getParam('secret');
                    $id     = Params::getParam('id');
                ...
                case 'add_comment':
                    ...
                    switch ($status) {
                        case -1: $msg = _m("Sorry, we couldn't save your comment. Please try again later");
                        ...
                        case 2:  $msg = _m('Your comment has been approved');
                            ...
                            break;
                        case 3:  $msg = _m('Please fill in the required field (e-mail)');
                                 osc_add_flash_warning_message($msg);
                            break;
                        case 4:  $msg = _m('Please enter a comment');
                                 osc_add_flash_warning_message($msg);
                            break;
                ...
                default:
                    ...
                    if ($item['b_active'] != 1) {
                        ...
                        if( osc_is_admin_user_logged_in() ) {
                            ...
                            osc_add_flash_warning_message( _m("The listing has been blocked or is awaiting moderation") );
                        } else {
                            $this->do400();
                            return;
                        }
                    ...
                    if(urlencode(strip_tags(strtolower(str_replace('+', '', str_replace(' ', '', urldecode($itemURI)))))) != urlencode(strip_tags(strtolower(str_replace(' ', '', urldecode($URI)))))) {
