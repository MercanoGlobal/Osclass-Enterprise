        ...
        function doModel()
        {
            switch( $this->action ) {
                case('login_post'):     //post execution for the login
										...
										if ( $email == '' ) {
											osc_add_flash_error_message( _m('Please provide an e-mail address') );
										...
										if($logged==0) {
											...
											if((time()-strtotime($user['dt_access_date']))>1200) { // EACH 20 MINUTES
												osc_add_flash_error_message(sprintf(_m('The user has not been validated yet. Would you like to re-send your <a href="%s">activation email?</a>'), osc_user_resend_activation_link($user['pk_i_id'], $user['s_email'])));
											} else {
												osc_add_flash_error_message(_m('The user has not been validated yet. Please check your email for the validation link.'));
											}
										} else if($logged==2) {
                ...
                case('forgot_post'):
                                        ...
                                        if($user['b_enabled'] == 1) {
                                            if(Params::getParam('new_password', false, false)==Params::getParam('new_password2', false, false)) {
                                                ...
                                                osc_add_flash_error_message( _m("Error, the password doesn't match"));
                                                $this->redirectTo(osc_forgot_user_password_confirm_url(Params::getParam('userId'), Params::getParam('code')));
                                            }
                                        } else {
                                            osc_add_flash_error_message( _m('Sorry, the link is not valid'));
