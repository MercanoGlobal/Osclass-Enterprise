    ...
    Class ItemActions
    {
        ...
        public function add()
        {
            ...
            $flash_error .=
                ...
                ((!osc_validate_max(number_format((float)$aItem[ 'price' ], 0, '', ''), 25)) ? _m('Price too long.') . PHP_EOL : '' ) .
                ...
                ((!osc_validate_name($contactName)) ? _m("The name is not valid") . PHP_EOL : '' ) .
                ((!osc_validate_email($contactEmail)) ? _m("Invalid e-mail.") . PHP_EOL : '' ) .
                ((!osc_validate_phone($contactPhone)) ? _m("Invalid phone.") . PHP_EOL : '' ) .
                ...
                ((((time() - (int)Session::newInstance()->_get('last_submit_item')) < osc_items_wait_time()) && !$this->is_admin) ? _m("Too fast. You should wait a little before publishing your ad.") . PHP_EOL : '' );
        ...
        function edit() {
            ...
            $flash_error .=
                ...
                ((!osc_validate_text($aItem['address'], 3, false))? _m("Address too short.") . PHP_EOL : '' ) .
                ((!osc_validate_max($aItem['address'], 100)) ? _m("Address too long.") . PHP_EOL : '' ) .
                ((!osc_validate_phone($aItem['contactPhone'])) ? _m('Invalid phone.') . PHP_EOL : '');
        ...
        public function prepareData( $is_add )
        {
            ...
            if ($aItem[ 'price' ] !== null) {
                ...
                if(is_numeric($price)) {
                    ...
                    $aItem['price'] = $price;   // will throw an error on number validation
                }
            }
                        ...
                        if ($error == UPLOAD_ERR_OK) {
                            ...
                            if(!is_dir($folder)) {
                                if (!@mkdir($folder, 0755, true)) {
                                    return 3; // PATH CANNOT BE CREATED
                                }
                            }
