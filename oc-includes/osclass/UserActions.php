 ...
 */
    ...
    Class UserActions
    {
        ...
        function add()
        {
            ...
            $str = $input['s_name'];
            $pattern = '/^[a-zA-Z0-9][a-zA-Z0-9\s.+&amp;-]+$/';   // Set up a regex to allow at least 1 letter, number, space and .+&- in the user name (blocks some Spam registration patterns).
            $validname = 0;
        ...
        function edit($userId)
        {
            ...
            if( !osc_validate_phone($input['s_phone_mobile']) ) {
                $flash_error .= _m('The phone is not valid') . PHP_EOL;
                $error[] = 11;
            }

            if( !osc_validate_phone($input['s_phone_land']) ) {
                $flash_error .= _m('The phone is not valid') . PHP_EOL;
                $error[] = 11;
            }
        ...
        function prepareData($is_add)
        {
            ...
            if(strtolower(substr($input['s_website'], 0, 4))!=='http') {
                $input['s_website'] = 'https://'.$input['s_website'];
            }
