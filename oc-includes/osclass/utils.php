...
function osc_sendMail($params) {
    ...
    if( array_key_exists('add_bcc', $params) ) {
        ...
        foreach((array)$params['add_bcc'] as $bcc) {
            try {
                $mail->AddBCC($bcc);
            } catch ( phpmailerException $e ) {
                continue;
...
function osc_do_upgrade() {
                    ...
                    if ($fail == 0) { // Everything is OK, continue
                        ...
                        if ($error_queries[0]) { // Everything is OK, continue
                            ...
                            if ($rm_errors == 0) {
                                ...
                                $message = __('Almost everything looks good! Your Osclass installation is up-to-date, but there were some errors removing temporary files. Please manually remove the "oc-content/downloads/oc-temp" folder');
    ...
    if ($error == 5) {
        $message .= "<br /><br />" . __('We encountered some errors while upgrading your database. The follwing queries failed:') . implode("<br />", $sql_error_msg);
    }
...
function osc_do_auto_upgrade() {
    echo json_encode(array('error' => 1, 'msg' => __('Version could not be verified')));
                    ...
                    if ($handle = opendir(osc_content_path() . 'downloads/oc-temp')) {
                        ...
                        if ($fail == 0) { // Everything is OK, continue
                            ...
                            if ($rm_errors == 0) {
                                ...
                                $message = __('Almost everything looks good! However there were some errors removing temporary files. Please manually remove the \"oc-content/downloads/oc-temp\" folder');
                                $error = 6; // Some errors removing files
                            }
                        } else {
                            $message = __('Problems when copying files. Please check your permissions. ');
