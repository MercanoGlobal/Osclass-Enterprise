    ...
    */
    ...
    function osc_item_contact_phone($only_number = true) {
        ...
        if(osc_reg_user_can_see_phone() == 1 && !osc_is_web_user_logged_in()) {
            ...
            if(!$only_number) {
                $phone = '<span title="' . osc_esc_html(__('You must login to see the phone number')) . '">' . $phone . '</span>';
            }
        }
    ...
    /**
     * Gets true if can show phone user at frontend, else return false
     *
     * @return boolean
     */
    ...
    */
    ...
    function osc_item_meta_value() {
        ...
        if($meta['e_type']=="DATEINTERVAL" || $meta['e_type']=="DATE") {
            ...
            if($value!='' && preg_match('/((([A-Za-z]{3,9}:(?:\/\/)?)(?:[-;:&=\+\$,\w]+@)?[A-Za-z0-9.-]+|(?:www.|[-;:&=\+\$,\w]+@)[A-Za-z0-9.-]+)((?:\/[\+~%\/.\w_-]*)?\??(?:[-\+=&;%@.\w_]*)#?(?:[\w]*))?)/', $value)) {
                if(stripos($value,'http://')!==false || stripos($value,'https://')!==false) {
                    return '<a target="_blank" rel="noopener nofollow" href="'.html_entity_decode($value, ENT_COMPAT, "UTF-8").'" >'.html_entity_decode($value, ENT_COMPAT, "UTF-8").'</a>';
                } else {
                    return '<a target="_blank" rel="noopener nofollow" href="http://'.html_entity_decode($value, ENT_COMPAT, "UTF-8").'" >'.html_entity_decode($value, ENT_COMPAT, "UTF-8").'</a>';
                }
            } else {
                return '';
            }
