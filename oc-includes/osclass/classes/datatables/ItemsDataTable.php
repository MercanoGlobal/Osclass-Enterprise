    ...
    class ItemsDataTable extends DataTable
    {
        ...
        private function processData($items)
        {
            if(!empty($items)) {
                ...
                foreach($items as $aRow) {
                    ...
                    $aResources = ItemResource::newInstance()->getResources( osc_item_id() );
                    foreach($aResources as $resource) {
                    $row['media'] = '<div style="text-align:center;"><a target="_blank" href="' . osc_admin_base_url(true) . '?page=media&amp;action=list&amp;resourceId=' . $aRow['pk_i_id'] . '"><img src="' . osc_apply_filter('resource_path', osc_base_url().$resource['s_path']).$resource['pk_i_id'].'_thumbnail.'.$resource['s_extension'] . '" style="max-width: 70px;" title="View media" /></a></div>';
                    }
        ...
        private function getDBParams($_get)
        {
            ...
            foreach($_get as $k => $v) {
                ...
                if($k == 'b_enabled' && $v != '') {
                    if($v == 'moderation') {
                        $this->mSearch->addItemConditions(DB_TABLE_PREFIX.'t_item.b_enabled = 0 AND ' . DB_TABLE_PREFIX . 't_item.b_active = 1');
                    } else {
