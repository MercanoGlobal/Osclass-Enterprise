 ...
 */
    ...
    class UsersDataTable extends DataTable
    {
        ...
        private function addTableHeader()
        {
            ...
            if(function_exists('mdh_avatar_thumbnail_url')) {  // Adds support for the Madhouse Avatar Plugin (can be changed with any other user avatar plugin)
                $this->addColumn('avatar', __('Avatar'));
            }
        ...
        private function processData($users)
        {
            if(!empty($users)) {
                ...
                foreach($users as $aRow) {
                    ...
                    $options[]  = '<a href="' . osc_admin_base_url(true) . '?page=users&action=login_user&amp;id='. $aRow['pk_i_id'] . '&amp;' . $csrf_token_url . '" target="_blank">' . sprintf(__('Log in as %s'), osc_highlight($aRow['s_name'], 20)) . '</a>';
                    ...
                    if(function_exists('mdh_avatar_thumbnail_url')) {  // Adds support for the Madhouse Avatar Plugin (can be changed with any other user avatar plugin)
                        $row['avatar'] = '<div style="text-align:center;"><img src="' . mdh_avatar_thumbnail_url($aRow['pk_i_id']) . '" style="max-width: 40px;" /></div>';
                    }
                    ...
                    $row['ip'] = "<a target=_blank href='https://tools.keycdn.com/geo?host=" . $aRow['s_access_ip'] . "'>" . $aRow['s_access_ip'] . "</a>" ;
                    ...
                    $row['items'] = '<a target=_blank href="' . osc_admin_base_url(true) . '?page=items&userId='. $aRow['pk_i_id'] .'&user='. $aRow['s_name'] .'">' . $aRow['i_items'] . '</a>'. $actions;
